...
The main Docker repo URL, common between Debian and RHEL systems.
    ...
    docker_apt_repository: "deb [arch={{ docker_apt_arch }}] {{ docker_repo_url }}/{{ ansible_facts["distribution"] | lower }} {{ ansible_facts["distribution_release"] }} {{ docker_apt_release_channel }}"
...
For configure swarm you should add variable `docker_swarm_configure: true` for all nodes, and `docker_swarm_manager: true` for managers. `inventory_hostname` must be equal to hostname (`ansible_facts['nodename']`) for correct remove nodes.
